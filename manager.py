...
class vmmManager(vmmGObjectUI):
    ...
    def __init__(self):
        ...
        # Add linked clone menu item to VM menu
        # VMActionMenu is a Gtk.Menu subclass, so we can directly add items
        self._add_linked_clone_menu_item()
    ...
    def _prompt_text(self, parent, title, default=""):
    ...
    def on_menu_linked_clone_activate(self, _menuitem=None):
        # Get the selected VM object
        vm = self.current_vm()
        if not vm:
            return

        # Get the actual libvirt domain name (not display title)
        # The VM object should have get_name() which returns the actual libvirt domain name
        src_name = None
        if hasattr(vm, "get_name"):
            src_name = vm.get_name()
        elif hasattr(vm, "name"):
            src_name = vm.name()
        elif hasattr(vm, "get_name_or_title"):
            src_name = vm.get_name_or_title()
        ...
        if not src_name:
            md = Gtk.MessageDialog(
                ...
                text="Cannot get VM name",
            )
            md.format_secondary_text("Could not determine the VM name.")
            md.run()
            md.destroy()
            return
        ...
        default_new = f"{src_name}-linked"
        new_name = self._prompt_text(self.topwin, "New linked clone name", default_new)
        if not new_name:
            return

        # Call our helper (safe shlex split to avoid shell injection worries)
        # Pass connection URI as 4th parameter if available
        # Use empty string for TARGET (3rd param) and connection URI (4th param) if needed
        if conn_uri:
            cmd = ["/usr/local/bin/vmm_linked_clone.sh", src_name, new_name, "", conn_uri]
        else:
            cmd = ["/usr/local/bin/vmm_linked_clone.sh", src_name, new_name]

        try:
            # Use list format instead of shell=True to avoid quote issues
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
            if result.returncode != 0:
                # Script failed - show error with actual output
                ...
                md = Gtk.MessageDialog(
                    ...
                    message_type=Gtk.MessageType.ERROR,
                    buttons=Gtk.ButtonsType.OK,
                    text="Linked clone failed",
                )
                cmd_str = " ".join([shlex.quote(str(arg)) for arg in cmd])
                md.format_secondary_text(f"Command: {cmd_str}\n\nError: {error_msg}")
                md.run()
                md.destroy()
                return
            # Refresh the UI list so the new VM appears
            # Force a connection refresh to pick up the new VM
            conn = vm.conn if hasattr(vm, 'conn') else None
            if conn:
                try:
                    conn.refresh()
                except Exception:
            ...
            md = Gtk.MessageDialog(
                ...
                message_type=Gtk.MessageType.ERROR,
                buttons=Gtk.ButtonsType.OK,
                text="Linked clone timeout",
            )
            md.format_secondary_text("The clone operation took too long and was cancelled.")
            md.run()
            md.destroy()
        except Exception as e:
            # Show an error dialog for unexpected errors
            md = Gtk.MessageDialog(
                transient_for=self.topwin,
                flags=0,
                message_type=Gtk.MessageType.ERROR,
            ...
            md.format_secondary_text(f"Unexpected error: {str(e)}")
            md.run()
            md.destroy()
    ...
    def _add_linked_clone_menu_item(self):
        """Add the linked clone menu item to the VM menu."""
        # Check if linked clone item already exists to avoid duplicates
        linked_item_exists = False
        for item in self.vmmenu.get_children():
            if hasattr(item, '_is_linked_clone_item'):
                linked_item_exists = True
                break
        
        if not linked_item_exists:
            # Add separator before our item
            separator = Gtk.SeparatorMenuItem()
            self.vmmenu.add(separator)
            separator.show()
            
            # Add linked clone menu item
            linked = Gtk.MenuItem.new_with_mnemonic("_Create Linked Clone…")
            linked.connect("activate", self.on_menu_linked_clone_activate)
            linked._is_linked_clone_item = True  # Mark it so we don't duplicate
            self.vmmenu.add(linked)
            linked.show()
